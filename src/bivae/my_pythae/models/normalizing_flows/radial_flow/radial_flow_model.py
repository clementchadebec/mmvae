...
class RadialFlow(BaseNF):
    ...
    def inverse(self, f: torch.Tensor, **kwargs) -> ModelOutput:
        """ Performs the inverse transformation """
        alpha = torch.exp(self.log_alpha)
        beta = -alpha + torch.log(1 + self.beta.exp())  # ensure invertibility
        R = torch.norm(f-self.x0, dim=-1, keepdim=True)
        delta = (alpha + beta - R)**2 + 4*alpha*R
        r = ((R-alpha-beta) + torch.sqrt(delta))/2
        r_z = (f - self.x0)/(1 + beta/(alpha + r))
