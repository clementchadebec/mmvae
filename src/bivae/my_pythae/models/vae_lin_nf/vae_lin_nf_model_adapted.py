...
class my_VAE_LinNF(VAE):
    """
    Adapted to be used in a JMVAE_NF
    ...
    def inverse_flow(self,z,**kwargs):
        """ Useful to compute q(z|x) when the z is given"""



        log_abs_det_jac = torch.zeros((z.shape[0],)).to(z.device)

        for layer in self.net:
            layer_output = layer.inverse(z)
            z = layer_output.out
            log_abs_det_jac += layer_output.log_abs_det_jac
