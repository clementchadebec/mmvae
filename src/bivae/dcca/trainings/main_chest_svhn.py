...
from bivae.dcca.models.chest_svhn import DCCA_CHEST_SVHN
...
if __name__ == '__main__':
    ...
    save_to = Path('../experiments/dcca/chest_svhn/')
    ...
    train_loader,test_loader, val_loader = CHEST_SVHN_DL('../data').getDataLoaders(batch_size=batch_size, dl_args=dlargs)
    ...
    ############
    wandb.init(project = 'DCCA_chest_svhn', entity = 'asenellart', config = {'batch_size' : batch_size,
    ...
    model = DCCA_CHEST_SVHN(outdim_size, use_all_singular_values, device=device).double()
    l_cca = None
    if apply_linear_cca:
        l_cca = linear_cca()
