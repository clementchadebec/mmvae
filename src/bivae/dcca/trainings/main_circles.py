...
from bivae.dcca.models import dcca_circles, load_dcca_circles
...
if __name__ == '__main__':
    ...
    save_to = Path('../experiments/dcca/circles/')
    ...
    epoch_num = 20
    batch_size = 800
    train_loader,test_loader, val_loader = CIRCLES_SQUARES_DL('../data/circles_squares/').getDataLoaders(batch_size=batch_size)
    ...
    ############
    wandb.init(project = 'DCCA_circles', entity = 'asenellart', config = {'batch_size' : batch_size,
    ...
    model = dcca_circles(outdim_size, use_all_singular_values, device=device)
    l_cca = None
    if apply_linear_cca:
        l_cca = linear_cca()
    ...
    visualize_umap(outputs_s[0], outputs_s[2], save_file=str(save_to) + '/embedding_0.png')
