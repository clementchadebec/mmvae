...
def compute_poe_for_all_subsets(mus, log_vars):
    ...
    for k in range(2, n_mod+1):
        ...
        for tup in combinations(range(n_mod), k):
            ...
            lnT = torch.stack([-log_vars_[i] for i in tup]) # Compute the inverse of variances
            joint_lnV = - torch.logsumexp(lnT, dim=0) # variances of the product of expert

            tensor_mus = torch.stack([mus_[i] for i in tup])
...
class MOEPOE(Multi_VAES):
    ...
    def forward(self, x):
        ...
        # Compute the poes and add them to the list of mus, along with the prior
        ...
        for i, mu in enumerate(mus):
            ...
            for m, vae in enumerate(self.vaes):
                ...
                lpx_z = self.px_z[m](recon, scale=1).log_prob(x[m])*self.lik_scaling[m]
                elbos += lpx_z  

            # And compute the KLD
            kld = kl_divergence(q, self.pz(*self.pz_params))
            elbos -= kld.sum()*self.params.beta_kl

        elbos = elbos/len(mus) # divide by the number of experts
        zs = zs/len(mus)
