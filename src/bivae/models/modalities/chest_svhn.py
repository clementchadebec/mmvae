...
from bivae.dataloaders import CHEST_SVHN_DL
...
from bivae.dcca.models.chest_svhn import load_dcca_chest_svhn
from bivae.analysis.classifiers.classifiers_medmnist import load_pneumonia_classifier
from bivae.analysis.classifiers import load_pretrained_svhn

from bivae.models.nn import Encoder_VAE_SVHN, Decoder_VAE_SVHN


class chest_svhn_utils():
    ...
    def __init__(self, params) -> None:
        ...
        self.lik_scaling = (3*32*32/(28*28),1) if params.llik_scaling == 0 else (1,1)
    ...
    def get_vaes(self,params):
        ...
        if params.dcca :
            ...
            #TODO
            self.dcca = load_dcca_chest_svhn(3)

            # Then add the flows
            encoder1 = TwoStepsEncoder(self.dcca[0], params)
            encoder2 = TwoStepsEncoder(self.dcca[1], params)
        else :
            encoder1, encoder2 = Encoder_ResNet_VAE_medmnist(vae_config1), Encoder_VAE_SVHN(vae_config2)


        # Define the decoders
        decoder1, decoder2 = Decoder_ResNet_AE_medmnist(vae_config1), Decoder_VAE_SVHN(vae_config2)
        ...
        vaes[1].modelName = 'svhn'
        
        return vaes
    
    def getDataLoaders(self, batch_size, shuffle=True, device="cuda", transform= transforms.ToTensor()):
        
        train, test, val = CHEST_SVHN_DL().getDataLoaders(batch_size, shuffle, device, transform=transform)
        return train, test, val
    
    def set_classifiers(self):

        self.classifiers = [load_pneumonia_classifier(), load_pretrained_svhn()]
        return 
