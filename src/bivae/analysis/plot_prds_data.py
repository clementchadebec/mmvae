...
for model in info.list:
    ...
    prds.append(np.load(path + 'prd_data_0.npy'))

# Plot
for i,type in enumerate(['Joint', 'Cond on 0', 'Cond on 1']):
    prds_to_plot = []
    f_beta = []
    f_beta_inv = []
    ...
    plot(prds_to_plot, labels = info.names, out_path='../experiments/comparison/prd_compare_{}.png'.format(type),legend_loc='upper right')
